...
public class WebSSOProfileImplTest extends SAMLTestBase {
    ...
    public void testPOSTSigned() throws Exception {
        ...
        storage.storeMessage((String) notNull(), (XMLObject) notNull());
        replyMock();
        AuthnRequest authnRequest = profile.initializeSSO(options, storage, request, response);
        verifyMock();
    ...
    public void testSentCorrectly() throws Exception {
        ...
        assertEquals("urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST", authnRequest.getProtocolBinding());
        verifyMock();
    }

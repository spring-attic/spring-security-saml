 ...
 *
 */
package org.springframework.security.saml2.serviceprovider.web.cache;
 ...
 * @author fhanik
 */
public class DefaultSaml2MetadataCache implements Saml2MetadataCache {
	...
	private TimebasedMap<String, Saml2ProviderNotFoundException> misses;

	public DefaultSaml2MetadataCache(Clock time,
	...
	public byte[] getMetadata(String uri, boolean skipSslValidation) {
		final Saml2ProviderNotFoundException hasMiss = misses.get(uri);
		if (nonNull(hasMiss)) {
			throw hasMiss;
		}
		...
		if (data == null) {
			try {
				...
				Saml2ProviderNotFoundException ex = new Saml2ProviderNotFoundException(
